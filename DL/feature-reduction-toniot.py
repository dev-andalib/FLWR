...
data = pd.read_csv("E:/IDS-FL-CSE400/IDS-FL-CSE400/DL/train_test_network.csv")



#preprocess

data = data.drop_duplicates() # remove all duplicate samples
data = data.drop(columns=['type']) # drop attack name

# remove duplicates, src, dst, ip and ports, timestamps
data =  data.drop(columns=['src_ip', 'dst_ip', 'src_port', 'dst_port'])
print(data.shape)

# Count of NaN values per column
nan_check = data.isna().sum()
print("NaN check:\n", nan_check)
# Count of Inf values per column
dash_check = (data == '-').sum()  
print("\nDash check:\n", dash_check)
# infinity check
num_cols = data.select_dtypes(include=[np.number]).columns
inf_check = np.isinf(data[num_cols]).sum()  
print("\nInfinity check:\n", inf_check)

# replace - with 0 and convert col to int for label encoding
for c in data.columns:
    if data[c].astype(str).str.contains('-').any() : # dash check 
        # Replace 
        data[c].replace('-', 0, inplace=True)
...
# Label Encoding and MinMaxScalar
...
# NaN check after splitting
data_x_df = pd.DataFrame(data_x)
# Check for NaN values in each column
print("NaN check in data_x:\n", data_x_df.isna().sum())
# Iterate over each column to replace NaN values
for c in data_x_df.columns:
    # Check for NaN and replace with 0
    if data_x_df[c].isna().any():
        data_x_df[c].fillna(0, inplace=True)       
        # Convert column to numeric if it is not already
        data_x_df[c] = pd.to_numeric(data_x_df[c], errors='coerce')
...
for fold, (train_idx, val_idx) in enumerate(skf.split(data_x, data_y)):
    ...
    pca["dt"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    pca["nb"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    pca["lg"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    pca["dnn"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    pca["cnn"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    pca["rnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
########################################################################################################################################################################################################
    ...
    loss, acc, dr, prec, f1, fpr, auc = dt.test(val, y_val)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    lda["dt"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = nb.test(val, y_val)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    lda["nb"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = lg.test(val, y_val)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    lda["lg"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    lda["dnn"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    lda["cnn"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    lda["rnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#########################################################################################################################################################################################################
    ...
    loss, acc, dr, prec, f1, fpr, auc = dt.test(val, y_val)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    ae["dt"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = nb.test(val, y_val)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    ae["nb"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = lg.test(val, y_val)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    ae["lg"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    ae["dnn"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    ae["cnn"].append([loss, acc, dr, prec, f1, fpr, auc])
    ...
    loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
    print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
    ae["rnn"].append([loss, acc, dr, prec, f1, fpr, auc]) 
...
avg_pca = calc_avj(pca)
avg_lda = calc_avj(lda)
avg_ae = calc_avj(ae)
