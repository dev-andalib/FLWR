...
data = pd.read_parquet('E:/IDS-FL-CSE400/CIC-ToN-IoT-V2.parquet')
print(data.head(3))
...
def train_AE(net, x_train, epochs=3, batch_size=1024, lr=0.001, wd=1e-4):
...
pca_NF = {"dt":[], "nb":[], "lg":[], "dnn":[], "cnn":[], "rnn":[]}
lda_NF = {"dt":[], "nb":[], "lg":[], "dnn":[], "cnn":[], "rnn":[]}
ae_NF  = {"dt":[], "nb":[], "lg":[], "dnn":[], "cnn":[], "rnn":[]}
...
#                                             torch.tensor(y_tr, dtype=torch.long)), batch_size=1024, shuffle=True)

#     val_loader = DataLoader(TensorDataset(torch.tensor(val, dtype=torch.float32),
#                                           torch.tensor(y_val, dtype=torch.long)), batch_size=1024, shuffle=False)
...
#     pca_NF["dt"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     pca_NF["nb"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     pca_NF["lg"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     pca_NF["dnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     pca_NF["cnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     pca_NF["rnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = dt.test(val, y_val)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     lda_NF["dt"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = nb.test(val, y_val)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     lda_NF["nb"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = lg.test(val, y_val)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     lda_NF["lg"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     lda_NF["dnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     lda_NF["cnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     lda_NF["rnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#         train_loader = DataLoader(TensorDataset(deep_x,  torch.tensor(y_tr, dtype=torch.long)), batch_size=1024, shuffle=True)

#         val_loader = DataLoader(TensorDataset(deep_x_val,torch.tensor(y_val, dtype=torch.long)), batch_size=1024, shuffle=False)
...
#     loss, acc, dr, prec, f1, fpr, auc = dt.test(val, y_val)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     ae_NF["dt"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = nb.test(val, y_val)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     ae_NF["nb"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = lg.test(val, y_val)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     ae_NF["lg"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     ae_NF["dnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     ae_NF["cnn"].append([loss, acc, dr, prec, f1, fpr, auc])
...
#     loss, acc, dr, prec, f1, fpr, auc = test(trained_m, val_loader, device=device)
#     print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
#     ae_NF["rnn"].append([loss, acc, dr, prec, f1, fpr, auc]) 
...
# avg_pca = calc_avj(pca_NF)
# avg_lda = calc_avj(lda_NF)
# avg_ae = calc_avj(ae_NF)
...
# Shuffle and split the data into 70% train and 30% test
x_train, x_test, y_train, y_test = train_test_split(data_x, data_y, test_size=0.3, random_state=42, stratify=data_y)

# Normalize the data (you should apply the same scaler to both train and test)
x_train = scaler.fit_transform(x_train)
x_test = scaler.transform(x_test)

# Perform PCA, LDA, and AE Feature Extraction (on training data)
print("\nUSING PCA / PRINCIPAL COMPONENT ANALYSIS")
x_train_pca = PCA_FUNC(x_train)
x_test_pca = PCA_FUNC(x_test)

# Perform LDA Feature Extraction
print("\nUSING LDA / LINEAR DISCRIMINANT ANALYSIS")
x_train_lda = LDA_FUNC(x_train, y_train)
x_test_lda = LDA_FUNC(x_test, y_test)
...
with torch.no_grad():
    ...
    # ENCODING                 
    deep_x_train = AE.encode(x_train_tensor)
    deep_x_test = AE.encode(x_test_tensor)

x_train_ae = deep_x_train.cpu().numpy()
x_test_ae = deep_x_test.cpu().numpy()


# Convert the data to PyTorch tensors
train_loader_pca = DataLoader(TensorDataset(torch.tensor(x_train_pca, dtype=torch.float32),
                                             torch.tensor(y_train, dtype=torch.long)), batch_size=1024, shuffle=True)

test_loader_pca = DataLoader(TensorDataset(torch.tensor(x_test_pca, dtype=torch.float32),
                                            torch.tensor(y_test, dtype=torch.long)), batch_size=1024, shuffle=False)

train_loader_lda = DataLoader(TensorDataset(torch.tensor(x_train_lda, dtype=torch.float32),
                                             torch.tensor(y_train, dtype=torch.long)), batch_size=1024, shuffle=True)

test_loader_lda = DataLoader(TensorDataset(torch.tensor(x_test_lda, dtype=torch.float32),
                                            torch.tensor(y_test, dtype=torch.long)), batch_size=1024, shuffle=False)

train_loader_ae = DataLoader(TensorDataset(torch.tensor(x_train_ae, dtype=torch.float32),
                                            torch.tensor(y_train, dtype=torch.long)), batch_size=1024, shuffle=True)

test_loader_ae = DataLoader(TensorDataset(torch.tensor(x_test_ae, dtype=torch.float32),
                                           torch.tensor(y_test, dtype=torch.long)), batch_size=1024, shuffle=False)

# Now, train and test all 18 models

# PCA models
def run_pca_models():
    models = ['dt', 'nb', 'lg', 'dnn', 'cnn', 'rnn']
    pca_NF = {"dt":[], "nb":[], "lg":[], "dnn":[], "cnn":[], "rnn":[]}

    print("\nRunning PCA Models:")
    for model_name in models:
        print(f"\nMetrics for {model_name} with PCA")

        if model_name == 'dt':
            dt = DecisionTree(max_depth=5)
            dt.fit(x_train_pca, y_train)
            loss, acc, dr, prec, f1, fpr, auc = dt.test(x_test_pca, y_test)
        elif model_name == 'nb':
            nb = NaiveBayes()
            nb.fit(x_train_pca, y_train)
            loss, acc, dr, prec, f1, fpr, auc = nb.test(x_test_pca, y_test)
        elif model_name == 'lg':
            lg = LogisticRegressor(max_iter=500)
            lg.fit(x_train_pca, y_train)
            loss, acc, dr, prec, f1, fpr, auc = lg.test(x_test_pca, y_test)
        elif model_name == 'dnn':
            dnn = DNN(20)
            trained_m = train(dnn, train_loader_pca, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_pca, device=device)
        elif model_name == 'cnn':
            cnn = CNN(20)
            trained_m = train(cnn, train_loader_pca, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_pca, device=device)
        elif model_name == 'rnn':
            rnn = RNN(20)
            trained_m = train(rnn, train_loader_pca, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_pca, device=device)

        pca_NF[model_name].append([loss, acc, dr, prec, f1, fpr, auc])
        print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
...
def run_lda_models():
    ...
    print("\nRunning LDA Models:")
    for model_name in models:
        print(f"\nMetrics for {model_name} with LDA")

        if model_name == 'dt':
            dt = DecisionTree(max_depth=5)
            dt.fit(x_train_lda, y_train)
            loss, acc, dr, prec, f1, fpr, auc = dt.test(x_test_lda, y_test)
        elif model_name == 'nb':
            nb = NaiveBayes()
            nb.fit(x_train_lda, y_train)
            loss, acc, dr, prec, f1, fpr, auc = nb.test(x_test_lda, y_test)
        elif model_name == 'lg':
            lg = LogisticRegressor(max_iter=500)
            lg.fit(x_train_lda, y_train)
            loss, acc, dr, prec, f1, fpr, auc = lg.test(x_test_lda, y_test)
        elif model_name == 'dnn':
            dnn = DNN(1)
            trained_m = train(dnn, train_loader_lda, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_lda, device=device)
        elif model_name == 'cnn':
            cnn = CNN(1)
            trained_m = train(cnn, train_loader_lda, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_lda, device=device)
        elif model_name == 'rnn':
            rnn = RNN(1)
            trained_m = train(rnn, train_loader_lda, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_lda, device=device)

        lda_NF[model_name].append([loss, acc, dr, prec, f1, fpr, auc])
        print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
...
def run_ae_models():
    ...
    print("\nRunning AE Models:")
    for model_name in models:
        print(f"\nMetrics for {model_name} with AE")

        if model_name == 'dt':
            dt = DecisionTree(max_depth=5)
            dt.fit(x_train_ae, y_train)
            loss, acc, dr, prec, f1, fpr, auc = dt.test(x_test_ae, y_test)
        elif model_name == 'nb':
            nb = NaiveBayes()
            nb.fit(x_train_ae, y_train)
            loss, acc, dr, prec, f1, fpr, auc = nb.test(x_test_ae, y_test)
        elif model_name == 'lg':
            lg = LogisticRegressor(max_iter=500)
            lg.fit(x_train_ae, y_train)
            loss, acc, dr, prec, f1, fpr, auc = lg.test(x_test_ae, y_test)
        elif model_name == 'dnn':
            dnn = DNN(20)
            trained_m = train(dnn, train_loader_ae, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_ae, device=device)
        elif model_name == 'cnn':
            cnn = CNN(20)
            trained_m = train(cnn, train_loader_ae, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_ae, device=device)
        elif model_name == 'rnn':
            rnn = RNN(20)
            trained_m = train(rnn, train_loader_ae, device=device)
            loss, acc, dr, prec, f1, fpr, auc = test(trained_m, test_loader_ae, device=device)

        ae_NF[model_name].append([loss, acc, dr, prec, f1, fpr, auc])
        print(f"Loss: {loss:.4f} , Accuracy: {acc:.4f}, DR (Recall): {dr:.4f}, Precision: {prec:.4f}, F1 Score: {f1:.4f}, FPR: {fpr:.4f}, AUC: {auc:.4f}")
...
# Run all the models
pca_results = run_pca_models()
lda_results = run_lda_models()
ae_results = run_ae_models()
